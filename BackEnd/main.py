...
def id_search(token, id):
    result = {}
    try:
        search = artist_search(token, id)
        result["name"] = search.get("artistname", "Artist")
        result["image"] = search.get("artistimage", "images/no_result.png")
        return result
    except Exception as e:
        print(e)
    try:
        search = track_search(token, id)
        result["name"] =  search.get("trackname", "Track")
        result["image"] = search.get("trackimage", "images/no_result.png")
        return result
    except Exception:
        print("not track")
    try:
        search = album_search(token, id)
        result["name"] = search.get("albumname", "Track")
        result["image"] = search.get("albumimage", "images/no_result.png")
        return result
    except:
